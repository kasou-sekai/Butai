...
Butai 是一个 macOS 原生 Spaces 增强工具。当前仓库包含可运行的 Phase 3 技术预览，覆盖工作区命名与切换、预设保存、补全、窗口布局恢复和首批应用适配器。
...
- 在所有 Spaces（包括全屏应用）显示不抢焦点的顶部浮窗，悬停后按系统顺序展开桌面名称和全屏应用标题；
- 全屏应用上的浮窗可设置为隐藏、移到顶部热区时显示或始终显示；
- 菜单栏入口、设置窗口、浮窗位置调整和系统兼容性检查；
- 从当前桌面的可见普通窗口保存默认预设和归一化布局；
- 在辅助功能权限可用时读取 Finder 窗口的真实文件夹路径；
